...
describe('按需图书与漫画封面适配', () => {
  ...
  it('即使目录在缓存异步命中期间变成混放，也不发布旧通用封面', async () => {
    ...
    vi.spyOn(mock.service.cache, 'get').mockImplementationOnce(async (key, signal) => { const value = await get(key, signal); started.resolve(); await gate.promise; return value })
  ...
  it('EPUB 内嵌或首图、漫画归档封面和目录直属图复用安全 Archive/像素检查', async () => {
    ...
    expect(mock.set.mock.calls.every(([key]) => key.startsWith('library:cache:thumbnail:'))).toBe(true)
    mock.service.destroy()
  })
