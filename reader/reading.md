...
scroll 的五页挂载窗口、四十一页局部轨道、惯性滚动不重写相同 scrollTop、未知尺寸锚点及相邻长短图补偿继续使用原算法。未知页的占位高度由「头部尺寸探测」提供：预加载器跟随阅读中心，只读每页开头少量字节解析真实宽高（目录页走独立小范围请求并保持 ±12 探测环；压缩包页只解压条目头部、读够即中止底层流约一个共享 1 MiB 分块，探测环收窄到预读窗口，已有整图字节直接复用），让快滚穿越未加载区时页码映射与真实内容一致，不再被估高带跳几十页。探测环外的未知页使用学习估高：有每页字节数（zip 中央目录或文件大小，打开时免费可得）时按「图高 ∝ 每页字节」比例估算并用已加载页中位数校准，条漫合集中短卡与长条页混排也不会互相带偏；无字节数回退平面中位数，学习前的初始估高不低于 2.2 个视口高。归档读取经共享分块在途去重，并发探测与整图预读不再重复下载同一分块。
