...
scroll 的五页挂载窗口、四十一页局部轨道、惯性滚动不重写相同 scrollTop、未知尺寸锚点及相邻长短图补偿继续使用原算法。未知页的占位高度由「头部尺寸探测」提供：预加载器跟随阅读中心，只读每页开头少量字节解析真实宽高（目录页走独立小范围请求，压缩包页只解压条目头部，已有整图字节直接复用），让快滚穿越未加载区时页码映射与真实内容一致，不再被估高带跳几十页；探测环外的未知页使用学习估高，学习前的初始估高不低于 2.2 个视口高。
