...
位置修正采用两种锚：停在**已知尺寸页**上时按该页像素偏移锚定，上方任何估高修正只补偿 scrollTop、画面不动；停在**占位页**上时使用「像素锚」——以最近一次停留的已知页（或程序化跳转目标）为基准，保留自该基准起的物理像素距离，穿越区的占位估高被修正后按真实高度反推落点页码，必要时自动换窗/换段。快滚穿越未探测区后的虚高页码不会被钉死，中间内容不会被永久跳过；程序化跳转会重置基准，不跨越整本书计算像素距离。
