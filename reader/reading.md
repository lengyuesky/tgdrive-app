...
scroll 的五页挂载窗口、四十一页局部轨道、惯性滚动不重写相同 scrollTop、未知尺寸锚点及相邻长短图补偿继续使用原算法。未知页的占位高度由「头部尺寸探测」提供：预加载器跟随阅读中心，只读每页开头少量字节解析真实宽高（目录页走独立小范围请求并保持 ±12 探测环；压缩包页只解压条目头部、读够即中止底层流约一个共享 1 MiB 分块，探测环收窄到预读窗口，已有整图字节直接复用），让快滚穿越未加载区时页码映射与真实内容一致。探测环外的未知页使用学习估高：**任何新知道尺寸的页（头部探测或整图）都是样本，首个样本即生效**——真实网络里探测总是先于整图到达，若只采整图样本，估高永远学不到、探测环外全靠初始估高，这正是条漫快滚跳几十页的根因之一。有每页字节数（zip 中央目录或文件大小，打开时免费可得）时按「图高 ∝ 每页字节」比例估算并用已知页中位数校准，条漫合集中短卡与长条页混排也不会互相带偏；无字节数回退平面中位数，学习前的初始估高不低于 2.2 个视口高。归档读取经共享分块在途去重，并发探测与整图预读不再重复下载同一分块。

位置修正采用两种锚：停在**已知尺寸页**上时按该页像素偏移锚定，上方任何估高修正只补偿坐标、画面不动；停在**占位页**上时使用「像素锚」——以最近一次停留的已知页（或程序化跳转目标）为基准，保留自该基准起的物理像素距离，穿越区的占位估高被修正后按真实高度反推落点页码，必要时自动换窗/换段。快滚穿越未探测区后的虚高页码不会被钉死，中间内容不会被永久跳过；程序化跳转会重置基准，不跨越整本书计算像素距离。

补偿坐标的时机由「滚动债」决定：手指拖动或惯性滚动进行中（`touchstart` 到 `touchend`，以及最后一个 `scroll` 事件后 180ms 内）**不改写 `scrollTop`**——iOS 在惯性中改写会直接打断惯性，且主线程读到的坐标落后于合成器，按旧坐标回写就是“滑动时往回跳一点”。此时锚点上方的一切高度修正（头部探测、估高学习、整图到达、向下换段）都吸收进上方占位：DOM 坐标 = 真实坐标 − 滚动债，上方占位按 DOM 坐标输出，锚点在 DOM 里纹丝不动。停稳后（`scrollend` 或静默计时）一次性结清：占位恢复真实高度并同步补偿坐标，同一任务内完成、画面不动。上方占位不能为负，吸收不下的余量、程序化跳转、视口尺寸变化以及**向上换段**（必须真的在上方腾出回看空间）仍改写坐标。同步测量前下方占位临时加长，避免布局缩短瞬间浏览器把 `scrollTop` 截断到新上限——那是一次无法吸收的跳动。
