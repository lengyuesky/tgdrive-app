...
- `apps/`：包目录，**仅保留每个官方应用的最新版本包**（当前共四包）：
  ...
  - `apps/books-1.3.6.tgapp`
  - `apps/comics-1.2.6.tgapp`
  - `apps/cinema-1.2.4.tgapp`

所有包的完整 manifest 必须与对应源码 `app.json` 一致。`SHA256SUMS` 包含：

```
<sha256>  apps/books-1.3.6.tgapp
<sha256>  apps/cinema-1.2.4.tgapp
<sha256>  apps/comics-1.2.6.tgapp
  ...
  "entries": [
    {
      ...
      "size": 1024,
      "url": "https://raw.githubusercontent.com/lengyuesky/tgdrive-app/main/apps/books-1.3.6.tgapp"
    }
  ]
}
```
