...
```bash
# 首次使用时安装测试浏览器；已有 Chromium 可改用下方环境变量指定
npx playwright install chromium
npm run test:comics

# 可选：使用本机已安装的浏览器
PLAYWRIGHT_CHROMIUM_EXECUTABLE=/path/to/chromium npm run test:comics
```

`tests/standalone/` 将当前漫画阅读器在内存中打包，注入实际阅读样式与合成 PNG，通过内存 SDK 模拟图片读取及延迟；不启动 HTTP 服务，浏览器网络请求全部阻断。用例覆盖：

- 真实滚轮连续跨页、快速回看，以及普通跨页不额外写入滚动坐标。
- 上方延迟图片缩短布局时，画面位置不受浏览器滚动截断影响。
- 六万像素长图在解码预算内优先保留，回看和视口变化保持页内进度。
- 手机触摸及松手后的真实惯性滚动，逐帧检查阅读位置单调前进。

测试产物默认位于系统临时目录（尊重 `TMPDIR`），可用 `PLAYWRIGHT_APPS_OUTPUT_DIR` 单独指定。该测试只验证独立阅读器，不代替下述跨库沙箱、SDK 和插件安装生命周期验收。

## 插件架构细节

- **`shorts/`**：原生纯 JavaScript 页面，体积轻量，无需打包工具介入，由 `package.mjs` 直出。
- **`books/`**：依赖 `reader/` 共享模块，支持长文本分段分页、EPUB 解压净化与 PDF 适宽渲染。
- **`comics/`**：依赖 `reader/` 共享模块，支持垂直连续滚动与横向单页翻阅。连续模式仅挂载相邻五页、保留至多四十一页的局部轨道；普通跨页只更新挂载节点，接近轨道边界时才换段。图片尺寸与 Blob 生命周期分离，显式预留高度，异步加载保留像素锚点，恢复进度和调整视口保留页内比例。
