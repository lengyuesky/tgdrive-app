...
test('漫画自然页序、两种模式、目录分章和有界归档读取', async ({ page }, info) => {
  ...
  const frame = page.frameLocator('iframe')
  await expect(frame.locator('.library-card .card-cover').first()).toBeVisible()
  await expect.poll(async () => {
    return await frame.locator('.library-card:has-text("自然页序.cbz") .cover-art').evaluate((node) => {
      return getComputedStyle(node).backgroundImage
    })
  }).toMatch(/blob:|url\(/)
  await frame.getByRole('button', { name: /自然页序.cbz/ }).click()
  await expect(frame.locator('#reading-status')).toHaveText('')
  ...
  await frame.locator('#preferences-toggle').click()
  await frame.locator('#mode').selectOption('page')
  ...
  await frame.locator('#back').click()
  await frame.getByRole('button', { name: /目录漫画/ }).click()
  await frame.getByRole('button', { name: /第1章/ }).click()
  await frame.locator('#read-folder').click()
  await expect(frame.locator('#reading-status')).toHaveText('')
  await expect(frame.locator('#toc option')).toHaveText(['1 · 1.png', '2 · 2.png', '3 · 10.png'])
  await frame.locator('#back').click()
  await frame.locator('#up').click(); await frame.locator('#up').click()
  await page.request.post('/__apps_fixture/media', { data: { enabled: false } })
  await frame.getByRole('button', { name: /大漫画.zip/ }).click()
  await expect(frame.locator('#reading-status')).toHaveText('', { timeout: 30000 })
  await expect.poll(() => frame.locator('img').first().evaluate((node) => (node as HTMLImageElement).naturalWidth)).toBe(768)
  ...
  await revealReader(page)
  await frame.locator('#preferences-toggle').click()
  await frame.locator('#mode').selectOption('scroll')
...
test('长漫画窗口、末页定位及 TXT 调整字号后保持字符位置', async ({ page }) => {
  ...
  await expect(frame.locator('#reading-status')).toHaveText('')
  await frame.locator('#jump').fill('100'); await frame.locator('#jump-button').click()
  await expect(frame.locator('#position')).toHaveText('100 / 200')
  await expect(frame.locator('.comic-page')).toHaveCount(5)
  await frame.locator('#jump').fill('200'); await frame.locator('#jump-button').click()
